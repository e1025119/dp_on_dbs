...
def solve_problem(cfg, cls, cnf_file, ptd_file, **kwargs):
    ...
    problem = cls(cnf_file,pool, **cfg["dpdb"], **kwargs)
    ...
    logger.info("Parsing input cnf file")
    input = problem.prepare_input(cnf_file)
    ...
    #p.stdin.close()
    tdr = TdReader.from_file(ptd_file)
    #p.wait()

    # solve it
    logger.info("Parsing input td file")
    td = TreeDecomp(tdr.num_bags, tdr.tree_width, tdr.num_orig_vertices, tdr.root, tdr.bags, tdr.adjacency_list) #TODO tdr.root auf 1 setzen
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(usage="%(prog)s [general options] -cnf cnf-input-file -td td-input file problem-type [problem specific-options]", formatter_class=MyFormatter)
    ...
    parser.add_argument("-cnf", "--cnf-file", dest="cnf_file", help="Cnf input file for the problem to solve", required=True)
    parser.add_argument("-ptd", "--ptd-file", dest="ptd_file", help="Proper TD input file for the problem to solve", required=True)
