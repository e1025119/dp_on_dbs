...
class Graph(nx.Graph):
    ...
    def write_gr(self, stream, relabel=True, copy=True):
        return self.write_graph(stream, relabel, copy, dimacs=False)

    def write_graph(self, stream, relabel=True, copy=True, dimacs=False):
        """
        :param stream: stream where to output the hypergraph
        :type stream: cString
        :param relabel: allow relabelling the nodes
        :type relabel: bool
